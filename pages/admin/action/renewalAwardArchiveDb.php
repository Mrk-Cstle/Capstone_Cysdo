...
$query =
    "SELECT renewal_award_archive.*, renewal.*, scholar.*
          FROM renewal
          ...
          $searchCondition
          ORDER BY renewal_award_archive.award_id ASC
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $tableHTML .= '<button class="deleteApplicant btn btn-sm btn-success" onclick="sendAction(\'' .  $row['award_id'] . '\', \'done\')">Restore</button> | ';
        $tableHTML .= '<button class="deleteApplicant btn btn-sm btn-danger" onclick="sendAction(\'' .  $row['award_id'] . '\', \'delete\')">Delete</button>';
    ...
    $countQuery = "SELECT COUNT(*) AS totalRows
              FROM renewal_award_archive
              JOIN renewal ON renewal.renewal_id = renewal_award_archive.renewal_id
              JOIN scholar ON scholar.scholar_id = renewal.scholar_id
              $searchCondition";
